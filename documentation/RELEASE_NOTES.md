...
**Fix: `compact(action=session)` seed now keeps the first full exchange, with a
progressive over-budget fallback.**

- **First exchange kept whole**: the new compacted chat's seed messages now include
  the first user message **and its assistant reply** (previously only the first user
  message was kept).
- **Progressive tail reduction**: when the estimated size exceeds 50% of the context
  window, the tail is first reduced from 2 exchanges to 1 and re-checked; only if it
  still exceeds the budget does compaction fall back to summary-only. Previously it
  dropped all messages immediately.
- Token estimation is recomputed at each step, so the "estimated" figure in the tool
  response and the `Modalità` line in the seed `.md` always reflect what was actually
  kept (`riassunto + primo scambio + ultime N exchanges`, or the summary-only note).
- Documentation: `documentation/compaction.md` updated to describe the new behaviour.

## v3.6.2 (2026-08-20)

**Fix: `compact(action=session)` robust with reasoning models and clean extraction.**

- **`chatCompletion` handles reasoning models** (e.g. Qwen3): LM Studio returns `content` empty
  and puts the "thinking" in `reasoning_content`, so the old code failed with
  "LLM returned empty content". Now it requests the chat template with thinking
  disabled, retries with the plain request, then with a much larger budget, and
  raises a clear error (model, finish reason, reasoning length) only if still empty.
- **Assistant extraction is now answer-only**: `parseConversation` keeps only each
  assistant message's final text block and excludes the reasoning narration, tool-call
  requests/results, `toolStatus` and `debugInfoBlock` steps. `compact(session)` therefore
  summarizes and carries only the real user turns and final answers (verified on a real
  Qwen3 conversation: 41k chars of reasoning → final answer only).
- `compact` tool description documents exactly where it reads
  (`messages[].versions[].steps[].content[]`) and what it extracts.

## v3.6.1 (2026-08-20)

**Patch: fixes from in-LM-Studio testing.**

- **No persistent console window in `--serve` mode**: `serve_stdio()` now applies
  `CREATE_NO_WINDOW` to the spawned node process (the launcher previously showed a
  console/conhost window for the whole session when started by an external MCP client).
- **Removed `outputSchema` from `compact` and `exec_job`**: strict clients (LM Studio)
  rejected `compact(memory|list|session)` and `exec_job(kill|list|clean)` with
  `-32600` because those actions return text only. Same fix pattern as v3.4.3.
- **`wiki_ingest(lint)` aligned with HOW_TO_USE**: dropped the frontmatter/confidence
  checks that contradicted the wiki's own "no YAML frontmatter" rule.
- **Embedding batch raised to 2048** (`--batch-size 2048` on llama-server): pages or
  chats with chunks above 512 tokens no longer fail with embedding HTTP 500.
- **`ingest_sessions` / `ingest_anythingllm` run in the background**: they return a
  `jobId` immediately and a new `rag(action=ingest_status, jobId=...)` reports the
  result, so long CPU-bound ingests no longer time out.

## v3.6.0 (2026-08-20)

**Session compaction (LM Studio only), RAG chat capture per client, wiki → RAG fixing, and no more PowerShell flash.**

- **`compact(action=session)`** (LM Studio only): given the chat title, writes a NEW
  `.conversation.json` (never touches the original) with `systemPrompt` = original +
  a structured summary of the whole conversation generated by the local model
  (`AURA_LLM_URL` / `AURA_LLM_MODEL`, default `http://localhost:1234`), and
  `messages` = first user message + last 2 exchanges verbatim. If the estimated size
  exceeds 50% of the context window (read from the chat file, `contextLength`, or
  `AURA_COMPACT_CONTEXT_LENGTH`), it keeps the summary only. Also writes a portable
  seed `.md` in `compacted-sessions/`.
- **Rename `anythingllm` → `anythingllm_chat_exporter`** and label it (AnythingLLM
  only). Its REST bridge logic is reused by the new `rag(action=ingest_anythingllm)`,
  which exports chats to `AnythingLLMSessions/` and indexes them into the RAG
  `sessions` collection — symmetric to `rag(action=ingest_sessions)` (LM Studio only).
- **`wiki_ingest(action=ingest_wiki)`**: fixes the curated wiki pages into the RAG
  `wiki` collection (upsert by page path; excludes `raw/`, `test/`, `ritest/`,
  `index.md`, `log.md`). Re-running updates in place.
- **LM Studio conversations path fix**: auto-detects `~/.cache/lm-studio/conversations`
  (LM Studio 0.4+) before the legacy `~/.lmstudio/conversations`.
- **PowerShell window fix**: `windowsHide: true` on every `powershell` invocation
  (`autoNotify` foreground check and `notify` beep/toast/balloon) and on `exec`
  spawns — no more flashing console windows.
- **Server instructions**: the MCP server now advertises `instructions` describing
  cross-tool priorities (LM Studio vs AnythingLLM chat capture) so agents pick the
  right tool chain.
